    ...
    for (let piece of pieces) {
            ...
            while (bitboard > 0n) {
                    ...
                    if (sourceSquare >= notToRawPos["a7"] && sourceSquare <= notToRawPos["h7"]) {
                        if (piece.getPromote()) {
                            whitePromotions.forEach(promotePiece => {
                                addMove(movesCopy, encodeMove(sourceSquare, targetSquare, piece.getID(), promotePiece, 0, 0, 0, 0));
                            });
                        }
                    } else {
                        ...
                        if ((sourceSquare >= notToRawPos["a2"] && sourceSquare <= notToRawPos["h2"])
                            ...
                            addMove(movesCopy, encodeMove(sourceSquare, targetSquare - 8, piece.getID(), 0, 0, 1, 0, 0));
                        }
                    }
                }

                // initialize pawn attacks bitboard
                attacks = piece.getPawnPieceState()[gameState.side][sourceSquare] & gameState.occupancies[colors.BLACK];
                while (attacks > 0n) {
                    ...
                    if (sourceSquare >= notToRawPos["a7"] && sourceSquare <= notToRawPos["h7"]) {
                        if (piece.getPromote()) {
                            whitePromotions.forEach(promotePiece => {
                                addMove(movesCopy, encodeMove(sourceSquare, targetSquare, piece.getID(), promotePiece, 1, 0, 0, 0));
                            });
                        }
                    } else {
                ...
                if (gameState.enpassant != -1) {
                    ...
                    if (enpassantAttacks) {
                        ...
                        addMove(movesCopy, encodeMove(sourceSquare, targetEnpassant, piece.getID(), 0, 1, 0, 1, 0));
                    }
                }
        ...
        if (piece.getColor() == colors.WHITE && piece.getKing()) {
            if (gameState.castle & BigInt(castlePieces.wk)) {
                if (!getBit(gameState.occupancies[colors.BOTH], notToRawPos["f1"])
                    ...
                    if (!isSquareAttacked(notToRawPos["e1"], colors.BLACK, 0) && !isSquareAttacked(notToRawPos["f1"], colors.BLACK, 0)) {
                        addMove(movesCopy, encodeMove(notToRawPos["e1"], notToRawPos["g1"], piece.getID(), 0, 0, 0, 0, 1));
                    }
                }
            }
            ...
            if (gameState.castle & BigInt(castlePieces.wq)) {
                if (!getBit(gameState.occupancies[colors.BOTH], notToRawPos["d1"]) && !getBit(gameState.occupancies[colors.BOTH], notToRawPos["c1"])
                    ...
                    if (!isSquareAttacked(notToRawPos["e1"], colors.BLACK, 0) && !isSquareAttacked(notToRawPos["d1"], colors.BLACK, 0)) {
                        addMove(movesCopy, encodeMove(notToRawPos["e1"], notToRawPos["c1"], piece.getID(), 0, 0, 0, 0, 1));
                    }
                }
            }
        }
        ...
        if (piece.getColor() == colors.BLACK && piece.getPawn()) {
            while (bitboard > 0n) {
                ...
                // initialize pawn attacks bitboard
                attacks = piece.getPawnPieceState()[gameState.side][sourceSquare] & gameState.occupancies[colors.WHITE];
                while (attacks > 0n) {
                    targetSquare = getLSFBIndex(attacks);
                    // pawn capture promotion
                    if (sourceSquare >= notToRawPos["a2"] && sourceSquare <= notToRawPos["h2"]) {
        ...
        if (piece.getColor() == colors.BLACK && piece.getKing()) {
            if (gameState.castle & BigInt(castlePieces.bk)) {
                if (!getBit(gameState.occupancies[colors.BOTH], notToRawPos["f8"])
                    ...
                    if (!isSquareAttacked(notToRawPos["e8"], colors.WHITE, 0) && !isSquareAttacked(notToRawPos["f8"], colors.WHITE, 0)) {
                        addMove(movesCopy, encodeMove(notToRawPos["e8"], notToRawPos["g8"], piece.getID(), 0, 0, 0, 0, 1));
                    }
                }
            }
            ...
            if (gameState.castle & BigInt(castlePieces.bq)) {
                if (!getBit(gameState.occupancies[colors.BOTH], notToRawPos["d8"])
                    ...
                    if (!isSquareAttacked(notToRawPos["e8"], colors.WHITE, 0) && !isSquareAttacked(notToRawPos["d8"], colors.WHITE, 0)) {
                        addMove(movesCopy, encodeMove(notToRawPos["e8"], notToRawPos["c8"], piece.getID(), 0, 0, 0, 0, 1));
                    }
                }
            }
        }
        ...
        if (piece.getLeaper() || piece.getSlider()) {
            ...
            if (piece.getRotationalMoveType() == "ROTATE" && piece.getLeaper()) {
                checkMove = currentMove % piece.getLeaperPieceState().length;
            }
            ...
            while (bitboard > 0n) {
                ...
                // get slider moves
                if (piece.getSlider()) {
                    attacks |= piece.getSlidingPieceAttacks(sourceSquare, gameState.occupancies[colors.BOTH]) & ((gameState.side == colors.WHITE)
