...
export const generateMove = (movesCopy: MoveList, piece: Piece) => {
    ...
    if (piece.getLeaper() || piece.getSlider()) {
        ...
        while (bitboard > 0n) {
            ...
            // get leaper moves
            if (piece.getLeaper()) {
                attacks |= piece.getLeaperPieceState()[piece.getColor()][checkMove][sourceSquare] & ((piece.getColor() == colors.WHITE) ? ~gameState.occupancies[colors.WHITE] : ~gameState.occupancies[colors.BLACK]);
            }

            // get slider moves
            if (piece.getSlider()) {
                attacks |= piece.getSlidingPieceAttacks(sourceSquare, gameState.occupancies[colors.BOTH], piece.straightConstraints, piece.diagonalConstraints) & ((gameState.side == colors.WHITE)
